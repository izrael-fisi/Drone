...
params_path="${VISION_NAV_PX4_PARAMS:-${1:-}}"
...
if [[ -z "$params_path" ]]; then
  cat >&2 <<EOF
Usage:
  VISION_NAV_PX4_PARAMS=/path/to/px4.params ./scripts/pi/check_px4_params.sh

Export PX4 parameters from QGroundControl or PX4 shell first. This script only
checks the exported file; it does not modify flight-controller parameters.
EOF
  exit 2
fi

args=(
  -m vision_nav.px4_params
  --params "$params_path"
)
