...
`tmux`, `cmake`, or PX4's Python build requirements, the capture wrapper still
creates the session scaffold, synthetic sender log, manual capture README, and
`px4_sitl_capture_prereqs.json` before exiting failed; rerun the same command
after fixing the prerequisite. The wrapper also prints
`__VISION_NAV_PX4_SITL_PREREQS__=...` so the prerequisite report can be copied
into setup notes. The generated fix commands include
`scripts/dev/setup_px4_sitl_prereqs.sh`, which is dry-run-first; use `--apply`
to install local tools/PX4 Python requirements and add `--clone-px4` only if
this workstation should create a PX4 checkout. Automated capture also records
PX4 build/startup output in `receiver_capture/px4_sitl_console.txt` and clears
stale listener/status captures before each attempt. Support bundles auto-detect the default
...
step, including the local `PX4 Prereq Setup` check before receiver capture and
the app-only `Load Next Field Condition` step after creating the field plan.
...
that must finish first. The field plan proof item uses the lightweight
template-plus-plan bootstrap command before the full Evidence Workflow command
for real terrain capture and registration. ROS replay validation also waits
until field terrain logs exist, and the bench phase asks for PX4 receiver
capture before support-bundle creation. It scans the
...
The Module Setup readiness card can copy immediately runnable next-action shell
commands, blocked follow-up commands, or a single command from its row, and the
JSON report includes the same machine-readable command bundle for support
tooling. When proof is incomplete, that bundle also includes
`guided_workflow_commands` with the ordered
...
immediate next-action commands, blocked follow-up commands, and pending field
replay capture/registration commands.
