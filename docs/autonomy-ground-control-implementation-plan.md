...
- Done: bench-readiness and autonomy-readiness GNSS-denied next actions now run
...
- Done: capture-ready field preflight now writes an executable
...
- Done: workflow-validation summaries now promote active required-step,
  downstream-blocked, superseded, and missing required-step counts to top-level
...
- Done: field-capture preflight diagnostics and downloaded support-bundle
  details now render the `field_log_capture_report.json` path beside the bundle,
  terrain log, and `runtime_status.json` paths, so support review can verify
  the complete capture artifact set without opening raw JSON.
