...
Module Setup also exposes a Bundle Diagnostic action that runs the same Pi-side
diagnostic before support-bundle capture and downloads the JSON report into the
desktop replay-cases folder. It also previews a bounded set of small image
artifacts from camera, debug, replay,
...
bundle/log/runtime-status paths, and copyable next-action commands. When
preflight writes a generated capture script or preflight-plus-capture command,
the summary exposes Copy Capture and Run Capture controls so Field Log Capture
uses the same preflight-approved command instead of reconstructing it from the
form. If bundle
...
need for the next proof registration. Module Setup shows the local draft
saved-at timestamp, provides a Reset Draft action when starting a fresh case,
and writes the draft metadata-readiness state into saved setup reports.
...
Accepted map matches are sent as MAVLink `ODOMETRY` by default, with local NED
position derived from the repo's local ENU measurement. This is the preferred
PX4 external-vision bench and final proof path. Set
`VISION_NAV_MAVLINK_MESSAGE=vision_position_estimate` only for pose-only
compatibility debugging. Rejected matches are logged but not sent. Runtime logs
include `external_position_health` snapshots with output status, send rate,
latency, skip reasons, and covariance warnings.
